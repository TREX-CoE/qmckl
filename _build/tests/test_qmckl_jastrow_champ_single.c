...
for (int elec = 0; elec < elec_num; elec++){
  ...
  for (int nw = 0; nw < walk_num; nw++) {
    ...
    //printf("diff %f\n", jastrow_een_new[nw]-jastrow_een_old[nw]);
    //printf("delta_een %f\n", delta_een[nw]);
    ...
    for (int nw = 0; nw < walk_num; nw++) {
      for (int i = 0; i < elec_num; i++) {
        for (int m = 0; m < 4; m++) {
          ...
          //printf("een_rescaled_single_e_gl[nw][l][m][i] %i %i %i %f\n", l, m, i,een_rescaled_single_e_gl[nw][l][m][i]);
          assert(fabs(een_rescaled_ee_gl[nw][l][i][m][elec] - een_rescaled_single_e_gl[nw][l][m][i]) < 1.e-12);
          assert(fabs(een_rescaled_ee_gl[nw][l][elec][m][i] - metric[m] * een_rescaled_single_e_gl[nw][l][m][i]) < 1.e-12);
        }
      }
    }
  }
...
for (int elec = 0; elec < elec_num; elec++){
  ...
  for (int nw = 0; nw < walk_num; nw++) {
    for (int m = 0; m < 3; m++) {
      //for (int i = 0; i < elec_num; i++) {
        printf("delta_een_g[%d][%d][%d] = %f\n", nw, m,elec, delta_een_g[nw][m][elec]);
        printf("een_g_[%d][%d][%d] = %f\n", nw, m,elec, een_g_new[nw][m][elec]-een_g_old[nw][m][elec]);
