...
Computing forces on nuclei (interatomic forces) is essential for geometry
optimization, molecular dynamics simulations, and understanding molecular
vibrations. In quantum Monte Carlo calculations, accurate force evaluation
requires derivatives of the wavefunction with respect to nuclear positions.

To calculate interatomic forces, derivatives with respect to nuclear coordinates
of the values, gradients, and Laplacians of both the Jastrow factor and the 
molecular orbitals (MO) are required. These derivatives propagate through the
local energy expression to yield the forces.

Furthermore, for calculations involving nonlocal pseudopotentials, the forces
also require derivatives of the single-electron move Jastrow values. The 
implementation of force calculations is one of the more complex aspects of
QMC codes, requiring careful attention to numerical accuracy and efficiency.
