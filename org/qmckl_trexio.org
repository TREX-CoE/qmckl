...
The [[https://trex-coe.github.io/trexio/trex.html][TREXIO library]] is a unified input/output system for quantum chemistry
calculations, designed to store and exchange wavefunction data in a standardized
format. TREXIO enables easy and efficient transfer of wavefunction parameters,
molecular geometries, basis sets, and other quantum chemical data between
different codes.

In this section, we provide high-level convenience functions to initialize
a QMCkl context by reading the required data from a TREXIO file. This allows
users to easily import wavefunction data from other quantum chemistry codes
that support TREXIO output, streamlining the workflow for QMC calculations.

The functions handle both TEXT and HDF5 backends of TREXIO, automatically
selecting the appropriate backend based on file format.
