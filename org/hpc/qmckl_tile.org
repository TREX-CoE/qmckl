...
Tiled arrays are an advanced data structure designed to improve computational
performance by optimizing memory access patterns and cache utilization. This
module implements tiled storage for matrices, which can significantly enhance
performance in computationally intensive linear algebra operations.

To increase performance, matrices may be stored as tiled arrays instead of
conventional two-dimensional arrays. In this scheme, a matrix is stored as a 
two-dimensional array of small matrices (forming a rank-4 tensor). This tiling
strategy provides several benefits:

1. **Improved data locality**: Operations on matrix tiles keep data in cache longer,
   reducing memory bandwidth requirements
2. **Better cache utilization**: Small tiles fit entirely in CPU cache, minimizing
   cache misses during computations
3. **BLAS3 optimization**: Enables use of highly optimized Level 3 BLAS operations
   (matrix-matrix operations) on individual tiles
4. **Sparse structure exploitation**: Allows efficient handling of sparse matrices
   by skipping empty tiles
