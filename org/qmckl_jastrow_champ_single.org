          ...
          if ( s_n_lpm > eps && n_num_lpm > eps ) {
            if (idx >= 0) {
              for (int64_t p = 1; p <= nz_[0] ; ++p) {
                int64_t j = nz_[p];
                accu = accu + (s_n_lpm * es_[j] - n_num_lpm * e_[j]) *n_[j];
              }
            } else {
              for (int64_t j = 0; j < elec_num; ++j) {
            ...
            if (idx >= 0) {
              for (int64_t p = 1; p <= nz_[0]; ++p) {
                int64_t j = nz_[p];
                accu = accu + s_n_lpm * es_[j] * n_[j];
              }
            } else {
              for (int64_t j = 0; j < elec_num; ++j) {
                accu = accu + s_n_lpm * es_[j];
              }
            }
...
#pragma omp simd reduction(+:accu)
...

        {
          ...
          if ( s_n_m > eps && n_num_m > eps ) {
            ...
            for (int64_t p = 1; p <= nz_[0]; ++p) {
              int64_t j = nz_[p];
              accu1 = accu1 + es_[j] * n_[j];
              accu2 = accu2 + e_[j] * n_[j];
//              accu = accu + (s_n_m * es_[j] - n_num_m * e_[j]) * n_[j];
            }
            accu = accu + s_n_m * accu1 - n_num_m*accu2;
          } else if ( s_n_m > eps ) {
            for (int64_t p = 1; p <= nz_[0]; ++p) {
              int64_t j = nz_[p];
              accu = accu + s_n_m * es_[j] * n_[j];
            }
          } else if ( n_num_m > eps ) {
            for (int64_t p = 1; p <= nz_[0]; ++p) {
              int64_t j = nz_[p];
              accu = accu - n_num_m * e_[j] * n_[j];
            }
          }
...
if (cord_num == 0) {
  ...
  void (*fptr) (qmckl_context context,
                const int64_t num,
                const int64_t walk_num,
                const int64_t elec_num,
                const int64_t nucl_num,
