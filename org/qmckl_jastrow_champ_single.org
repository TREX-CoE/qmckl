                                 ...
                                 qmckl_jastrow_champ_single_struct* dest)
{
  ...
  if (src->coord.data != NULL && src->coord.size[0] > 0 && src->coord.size[1] > 0) {
    int64_t size = src->coord.size[0] * src->coord.size[1];
    dest->coord.data = (double*) qmckl_malloc(context, size * sizeof(double));
    if (dest->coord.data == NULL) {
      return QMCKL_ALLOCATION_FAILED;
    }
    memcpy(dest->coord.data, src->coord.data, size * sizeof(double));
