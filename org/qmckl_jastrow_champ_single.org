...
qmckl_compute_jastrow_champ_delta_p_hpc(const qmckl_context context,
                                        ...
                                        const int64_t nucl_num,
                                        const int64_t cord_num,
                                        const double* restrict een_rescaled_n,         /* (elec_num, nucl_num, 0..cord_num, walk_num) */
                                        const double* restrict een_rescaled_e,         /* (elec_num, elec_num, 0..cord_num, walk_num) */
                                        const double* restrict een_rescaled_single_n,  /* (nucl_num, 0..cord_num, walk_num) */
                                        const double* restrict een_rescaled_single_e,  /* (elec_num, 0..cord_num, walk_num) */
                                        double* restrict delta_p                       /* (elec_num, nucl_num, 0..cord_num, 0..cord_num-1, walk_num) */
                                        )
{
  ...
  const int64_t D_elec = elec_num;
  const int64_t D_nucl = nucl_num;
  const int64_t D_c    = cord_num + 1;  /* 0..cord_num */
  const int64_t D_i    = cord_num;      /* 0..cord_num-1 */
  const int64_t D_walk = walk_num;

  /* Index helpers for Fortran-order arrays */
#define EEN_N(j,a,c,nw)  (een_rescaled_n       [ (j) + D_elec * ( (a) + D_nucl * ( (c) + D_c * (nw) ) ) ])
#define EEN_E(j,k,i,nw)  (een_rescaled_e       [ (j) + D_elec * ( (k) + D_elec * ( (i) + D_c * (nw) ) ) ])
#define EEN_SN(a,c,nw)   (een_rescaled_single_n[ (a) + D_nucl * ( (c) + D_c * (nw) ) ])
#define EEN_SE(j,i,nw)   (een_rescaled_single_e[ (j) + D_elec * ( (i) + D_c * (nw) ) ])
#define DELTA_P(j,a,c,i,nw) (delta_p           [ (j) + D_elec * ( (a) + D_nucl * ( (c) + D_c * ( (i) + D_i * (nw) ) ) ) ])

//  const double eps = qmckl_get_numprec_epsilon(context);
  const int64_t num_idx = num_in; /* 0-based electron index corresponding to Fortran's num = num_in + 1 */

  /* Workspace (vector over electrons) */
  /* C99 VLA on the stack; if not desired, replace with heap allocation. */
  double een_rescaled_delta_e_stack[ (size_t) (D_elec > 0 ? D_elec : 1) ];
  double* restrict een_rescaled_delta_e = een_rescaled_delta_e_stack;

  for (int64_t nw = 0; nw < D_walk; ++nw) {

    /*
,*/
    // i = 0 case: c = 1..cord_num (Fortran) => c = 1..cord_num (C, 0-based c is used directly) 
    for (int64_t c = 1; c <= cord_num; ++c) {
      for (int64_t a = 0; a < D_nucl; ++a) {
        const double dn = EEN_SN(a, c, nw) - EEN_N(num_idx, a, c, nw);

#ifdef HAVE_OPENMP
#pragma omp simd
#endif
        for (int64_t j = 0; j < D_elec; ++j) {
          DELTA_P(j, a, c, 0, nw) = dn;
        }
        DELTA_P(num_idx, a, c, 0, nw) = 0.0;
      }
    }

    // c = 0 case: i = 1..cord_num-1 
...
#endif
      for (int64_t j = 0; j < D_elec; ++j) {
        een_rescaled_delta_e[j] = EEN_SE(j, i, nw) - EEN_E(j, num_idx, i, nw);
        dn_sum += een_rescaled_delta_e[j];
      }

      // delta_p(:,a,0,i,nw) = een_rescaled_delta_e(:) for all nuclei a 
      for (int64_t a = 0; a < D_nucl; ++a) {
#ifdef HAVE_OPENMP
#pragma omp simd
#endif
        for (int64_t j = 0; j < D_elec; ++j) {
          DELTA_P(j, a, 0, i, nw) = een_rescaled_delta_e[j];
        }
        DELTA_P(num_idx, a, 0, i, nw) += dn_sum;
      }
    }

    /* General case: i = 1..cord_num-1 and c = 1..cord_num-i */
    for (int64_t i=1; i<cord_num; ++i) {

      /* recompute een_rescaled_delta_e for this i */
#ifdef HAVE_OPENMP
#pragma omp simd
#endif
      for (int64_t j = 0; j < D_elec; ++j) {
      ...
      for (int64_t c = 1; c <= cord_num - i; ++c) {
        for (int64_t a = 0; a < D_nucl; ++a) {
          // delta_p(:,a,c,i,nw) = een_rescaled_e(:,num,i,nw)*dn + een_rescaled_delta_e(:)*dn2 

          const double dn2 = EEN_SN(a, c, nw);
          const double dn  = EEN_SN(a, c, nw) - EEN_N(num_idx, a, c, nw);
...
#endif
          for (int64_t j = 0; j < D_elec; ++j) {
            DELTA_P(j, a, c, i, nw) = EEN_E(j, num_idx, i, nw) * dn + een_rescaled_delta_e[j] * dn2;
          }

        }
      }
      qmckl_dgemv(context, 'T',  elec_num, nucl_num*(cord_num-i), 1.0, &(EEN_N(0,0,1,nw)), elec_num,
                    een_rescaled_delta_e, 1, 1.0, &(DELTA_P(num_idx,0,1,i,nw)), elec_num);
...
integer function qmckl_compute_jastrow_champ_factor_single_een_doc( &
  ...
  double precision :: accu, accu2, accu3, accu4, cn, eps
  integer*8                           :: LDA, LDB, LDC
     ...
     do n = 1, dim_c_vector
        ...
        do a = 1, nucl_num
           ...
           do i = 1, elec_num
              ...
              accu2 = accu2 - een_rescaled_n(i,a,m+l,nw) * een_rescaled_e(i,num,k,nw)
              accu3 = accu3 + een_rescaled_n(i,a,m,nw) * een_rescaled_single_e(i,k,nw)
              accu4 = accu4 - een_rescaled_n(i,a,m,nw) * een_rescaled_e(i,num,k,nw)
           end do

           accu = accu * een_rescaled_single_n(a,m,nw) + accu2 * &
                een_rescaled_n(num,a,m,nw) + accu3 * &
                een_rescaled_single_n(a,m+l,nw) + &
                accu4 * een_rescaled_n(num,a,m+l,nw)
  ...
  do nw =1, walk_num
     ...
     do n = 1, dim_c_vector
        ...
        do a = 1, nucl_num
           ...
           if ((dabs(een_rescaled_single_n(a,l+m,nw)) > eps).and.(dabs(een_rescaled_n(num,a,l+m,nw)) > eps)) then
              if (idx > 0) then
           ...
           else if (dabs(een_rescaled_single_n(a,l+m,nw)) > eps) then
           ...
           else if (dabs(een_rescaled_n(num,a,l+m,nw)) > eps) then
           ...
           if ((dabs(een_rescaled_single_n(a,m,nw)) > eps).and.(dabs(een_rescaled_n(num,a,m,nw)) > eps)) then
           ...
           else if (dabs(een_rescaled_single_n(a,m,nw)) > eps) then
           ...
           else if (dabs(een_rescaled_n(num,a,m,nw)) > eps) then
...
end function qmckl_compute_jastrow_champ_factor_single_een_hpc_f
     #+end_src

 #+begin_src c :comments org :tangle (eval c) :noweb yes :exports none
qmckl_exit_code
qmckl_compute_jastrow_champ_factor_single_een_hpc(
                                                  const qmckl_context context,
                                                  const int64_t num,
                                                  const int64_t walk_num,
                                                  const int64_t elec_num,
                                                  ...
                                                  )
{
  ...
  /* Allocate non_zeros as a flat buffer:
     dimensions in Fortran: non_zeros(0:elec_num, nucl_num, 2)
     We'll store as: stride_e = (elec_num + 1)
     layout: [t][a][pos], where t=0..1 (third dim), a=0..nucl_num-1, pos=0..elec_num
     We keep pos=0 as the count like your Fortran code; entries stored at pos=1..count */
  const int64_t stride_e = elec_num + 1;
  const int64_t t_count = 2;
  const size_t total_nonzeros_elems = (size_t)stride_e * (size_t)nucl_num * (size_t)t_count;
  int64_t * non_zeros = (int64_t *) malloc(total_nonzeros_elems * sizeof(int64_t));
...
#define IDX_CVECTOR(a,n)  ((size_t)(a) + (size_t)(n) * (size_t)nucl_num)
  /* lkpm_combined_index: shape (dim_c_vector, 4) */
#define IDX_LKPM(n,col)   ((size_t)(n) + (size_t)(col) * (size_t)dim_c_vector)
  /* een_rescaled_n: (elec_num, nucl_num, cord_dim, walk_num) */
#define IDX_EEN_N(j,a,l,nw) (                                           \
                              (size_t)(j) + (size_t)(a) * (size_t)elec_num + \
                              (size_t)(l) * (size_t)elec_num * (size_t)nucl_num + \
                              (size_t)(nw) * (size_t)elec_num * (size_t)nucl_num * (size_t)cord_dim )
  /* een_rescaled_e: (elec_num, elec_num, cord_dim, walk_num) */
#define IDX_EEN_E(j,col,l,nw) (                                         \
                                (size_t)(j) + (size_t)(col) * (size_t)elec_num + \
                                (size_t)(l) * (size_t)elec_num * (size_t)elec_num + \
                                (size_t)(nw) * (size_t)elec_num * (size_t)elec_num * (size_t)cord_dim )
  /* een_rescaled_single_n: (nucl_num, cord_dim, walk_num) */
#define IDX_EEN_SING_N(a,l,nw) (                                        \
                                 (size_t)(a) + (size_t)(l) * (size_t)nucl_num + \
                                 (size_t)(nw) * (size_t)nucl_num * (size_t)cord_dim )
  /* een_rescaled_single_e: (elec_num, cord_dim, walk_num) */
#define IDX_EEN_SING_E(j,l,nw) (                                        \
                                 (size_t)(j) + (size_t)(l) * (size_t)elec_num + \
                                 (size_t)(nw) * (size_t)elec_num * (size_t)cord_dim )

  const double eps = qmckl_get_numprec_epsilon(context);

  /* Parallelize over walkers: each walker computes delta_een[nw] independently. */
#ifdef HAVE_OPENMP
#pragma omp parallel
#endif
  {
    /* Allocate per-walker temporary non_zeros array on heap (thread-private). */
    int64_t * nz = (int64_t *) malloc(total_nonzeros_elems * sizeof(int64_t));
    assert (nz != NULL);
...
#pragma omp for
    ...
    for (int64_t nw = 0; nw < walk_num; ++nw) {
      ...
      for (int64_t a = 0; a < nucl_num; ++a) {
        int64_t k = 0;  /* count for tier 1 (matching m) */
        int64_t p = 0;  /* count for tier 2 (matching l+m) */
        for (int64_t j = 0; j < elec_num; ++j) {
          const double v_m = een_rescaled_n[IDX_EEN_N(j, a, 2, nw)];
          if (v_m > eps) {
            k++; p++;
            nz[ /* non_zeros(k,a,1) */ (size_t)1 + (size_t)k - 1 + (size_t)a * (size_t)stride_e + (size_t)0 * (size_t)stride_e * (size_t)nucl_num ] = j;
            nz[ /* non_zeros(p,a,2) */ (size_t)1 + (size_t)p - 1 + (size_t)a * (size_t)stride_e + (size_t)1 * (size_t)stride_e * (size_t)nucl_num ] = j;
          } else {
            const double v_lm = een_rescaled_n[IDX_EEN_N(j, a, 1, nw)];
            if (v_lm > eps) {
              k++;
              nz[ /* non_zeros(k,a,1) */ (size_t)1 + (size_t)k - 1 + (size_t)a * (size_t)stride_e + (size_t)0 * (size_t)stride_e * (size_t)nucl_num ] = j;
            }
        ...
        /* store counts in position 0 as in Fortran */
        nz[ /* non_zeros(0,a,1) */ (size_t)0 + (size_t)a * (size_t)stride_e + (size_t)0 * (size_t)stride_e * (size_t)nucl_num ] = k;
        nz[ /* non_zeros(0,a,2) */ (size_t)0 + (size_t)a * (size_t)stride_e + (size_t)1 * (size_t)stride_e * (size_t)nucl_num ] = p;
      } /* end build non_zeros for walker */
      ...
      for (int64_t n = 0; n < dim_c_vector; ++n) {
        ...
        const int64_t k_idx = lkpm_combined_index[ IDX_LKPM(n, 1) ]; /* we avoid naming collision with loop var k above */
        ...
        for (int64_t a = 0; a < nucl_num; ++a) {
          ...
          /* first block: idx = min(m,2) in Fortran */
          int64_t idx = (m < 2) ? m : 2;
          ...
          if ( fabs(s_n_lpm) > eps && fabs(n_num_lpm) > eps ) {
            if (idx > 0) {
              int64_t t = idx - 1; /* Fortran's idx 1 => t=0, idx 2 => t=1 */
              int64_t count = nz[ (size_t)0 + (size_t)a * (size_t)stride_e + (size_t)t * (size_t)stride_e * (size_t)nucl_num ];
              for (int64_t p = 1; p <= count; ++p) {
                int64_t j = nz[ (size_t)p + (size_t)a * (size_t)stride_e + (size_t)t * (size_t)stride_e * (size_t)nucl_num ];
                double term = s_n_lpm * een_rescaled_single_e[ IDX_EEN_SING_E(j, k_idx, nw) ] -
                  n_num_lpm * een_rescaled_e[ IDX_EEN_E(j, num, k_idx, nw) ];
                accu += term * een_rescaled_n[ IDX_EEN_N(j, a, m, nw) ];
              }
            } else {
#ifdef HAVE_OPENMP
#pragma omp simd reduction(+:accu)
          ...
          } else if ( fabs(s_n_lpm) > eps ) {
            if (idx > 0) {
              int64_t t = idx - 1;
              int64_t count = nz[ (size_t)0 + (size_t)a * (size_t)stride_e + (size_t)t * (size_t)stride_e * (size_t)nucl_num ];
              for (int64_t p = 1; p <= count; ++p) {
                int64_t j = nz[ (size_t)p + (size_t)a * (size_t)stride_e + (size_t)t * (size_t)stride_e * (size_t)nucl_num ];
                accu += s_n_lpm * een_rescaled_single_e[ IDX_EEN_SING_E(j, k_idx, nw) ] * een_rescaled_n[ IDX_EEN_N(j, a, m, nw) ];
              }
            } else {
#ifdef HAVE_OPENMP
#pragma omp simd reduction(+:accu)
#endif
              for (int64_t j = 0; j < elec_num; ++j) {
                accu += s_n_lpm * een_rescaled_single_e[ IDX_EEN_SING_E(j, k_idx, nw) ];
              }
            }
          } else if ( fabs(n_num_lpm) > eps ) {
            if (idx > 0) {
              int64_t t = idx - 1;
              int64_t count = nz[ (size_t)0 + (size_t)a * (size_t)stride_e + (size_t)t * (size_t)stride_e * (size_t)nucl_num ];
              for (int64_t p = 1; p <= count; ++p) {
                int64_t j = nz[ (size_t)p + (size_t)a * (size_t)stride_e + (size_t)t * (size_t)stride_e * (size_t)nucl_num ];
                accu -= n_num_lpm * een_rescaled_e[ IDX_EEN_E(j, num, k_idx, nw) ] * een_rescaled_n[ IDX_EEN_N(j, a, m, nw) ];
              }
            } else {
#ifdef HAVE_OPENMP
#pragma omp simd reduction(+:accu)
#endif
              for (int64_t j = 0; j < elec_num; ++j) {
                accu -= n_num_lpm * een_rescaled_e[ IDX_EEN_E(j, num, k_idx, nw) ];
              }
            }
          }

          idx = ((l + m) < 2) ? (l + m) : 2;
          /* reuse variables with new positions */
          const double s_n_m = een_rescaled_single_n[ IDX_EEN_SING_N(a, m, nw) ];
          const double n_num_m = een_rescaled_n[ IDX_EEN_N( num, a, m, nw) ];

          if ( fabs(s_n_m) > eps && fabs(n_num_m) > eps ) {
            int64_t t = idx - 1;
            int64_t count = nz[ (size_t)0 + (size_t)a * (size_t)stride_e + (size_t)t * (size_t)stride_e * (size_t)nucl_num ];
            for (int64_t p = 1; p <= count; ++p) {
              int64_t j = nz[ (size_t)p + (size_t)a * (size_t)stride_e + (size_t)t * (size_t)stride_e * (size_t)nucl_num ];
              double term = s_n_m * een_rescaled_single_e[ IDX_EEN_SING_E(j, k_idx, nw) ] -
                n_num_m * een_rescaled_e[ IDX_EEN_E(j, num, k_idx, nw) ];
              accu += term * een_rescaled_n[ IDX_EEN_N(j, a, l + m, nw) ];
            }
          } else if ( fabs(s_n_m) > eps ) {
            int64_t t = idx - 1;
            int64_t count = nz[ (size_t)0 + (size_t)a * (size_t)stride_e + (size_t)t * (size_t)stride_e * (size_t)nucl_num ];
            for (int64_t p = 1; p <= count; ++p) {
              int64_t j = nz[ (size_t)p + (size_t)a * (size_t)stride_e + (size_t)t * (size_t)stride_e * (size_t)nucl_num ];
              accu += s_n_m * een_rescaled_single_e[ IDX_EEN_SING_E(j, k_idx, nw) ] * een_rescaled_n[ IDX_EEN_N(j, a, l + m, nw) ];
            }
          } else if ( fabs(n_num_m) > eps ) {
            int64_t t = idx - 1;
            int64_t count = nz[ (size_t)0 + (size_t)a * (size_t)stride_e + (size_t)t * (size_t)stride_e * (size_t)nucl_num ];
            for (int64_t p = 1; p <= count; ++p) {
              int64_t j = nz[ (size_t)p + (size_t)a * (size_t)stride_e + (size_t)t * (size_t)stride_e * (size_t)nucl_num ];
              accu -= n_num_m * een_rescaled_e[ IDX_EEN_E(j, num, k_idx, nw) ] * een_rescaled_n[ IDX_EEN_N(j, a, l + m, nw) ];
            }
          }
...
qmckl_exit_code
qmckl_compute_jastrow_champ_factor_single_een_hpc (const qmckl_context context,
                              const int64_t num,
                              const int64_t walk_num,
                              const int64_t elec_num,
                              const int64_t nucl_num,
...
integer(qmckl_exit_code) function qmckl_compute_jastrow_champ_delta_p_gl_hpc( &
  ...
  integer*8 :: i, a, j, l, k, p, m, n, nw, num
  double precision :: tmp, cummu, accu(3)
  integer*8        :: LDA, LDB, LDC
        ...
        do l=1, cord_num-m
           do a = 1, nucl_num
              ...
              do i = 1, elec_num
                 ...
                 delta_p_gl(i,a,1,l,m,nw) = -een_rescaled_e_gl(i,1,num,m,nw) * een_re_n &
                      - een_rescaled_single_e_gl(1,i,m,nw) * een_re_single_n
                 delta_p_gl(i,a,2,l,m,nw) = -een_rescaled_e_gl(i,2,num,m,nw) * een_re_n &
                      - een_rescaled_single_e_gl(2,i,m,nw) * een_re_single_n
                 delta_p_gl(i,a,3,l,m,nw) = -een_rescaled_e_gl(i,3,num,m,nw) * een_re_n &
                      - een_rescaled_single_e_gl(3,i,m,nw) * een_re_single_n
                 
                 accu(1) = accu(1) + delta_e_gl(i,1) * een_rescaled_n(i,a,l,nw)
                 accu(2) = accu(2) + delta_e_gl(i,2) * een_rescaled_n(i,a,l,nw)
                 accu(3) = accu(3) + delta_e_gl(i,3) * een_rescaled_n(i,a,l,nw)
              end do
              delta_p_gl(num,a,1,l,m,nw) = delta_p_gl(num,a,1,l,m,nw) + accu(1)
              delta_p_gl(num,a,2,l,m,nw) = delta_p_gl(num,a,2,l,m,nw) + accu(2)
              delta_p_gl(num,a,3,l,m,nw) = delta_p_gl(num,a,3,l,m,nw) + accu(3)
...
integer(qmckl_exit_code) function qmckl_compute_jastrow_champ_factor_single_een_gl_doc( &
  ...
  double precision :: accu, accu1, accu2, accu3, accu4, accu5, accu6, accu7, accu8, cn
  ...
  do nw = 1, walk_num
    do n = 1, dim_c_vector
      ...
      do a = 1, nucl_num
        ...
        do kk = 1, 3
          do i = 1, elec_num
            accu1 = een_rescaled_n(i,a,m+l,nw) * een_rescaled_single_e_gl(kk,i,k,nw) 
            accu2 = een_rescaled_n(i,a,m+l,nw) * een_rescaled_e_gl(i,kk,num,k,nw)
            accu3 = een_rescaled_n(i,a,m,nw) * een_rescaled_single_e_gl(kk,i,k,nw)
            accu4 = een_rescaled_n(i,a,m,nw) * een_rescaled_e_gl(i,kk,num,k,nw)

            accu5 = een_rescaled_n(i,a,m+l,nw) * een_rescaled_single_e(i,k,nw) 
            accu6 = - een_rescaled_n(i,a,m+l,nw) * een_rescaled_e(i,num,k,nw) 
            accu7 = een_rescaled_n(i,a,m,nw) * een_rescaled_single_e(i,k,nw) 
            accu8 = - een_rescaled_n(i,a,m,nw) * een_rescaled_e(i,num,k,nw)

            accu = accu1 * een_rescaled_single_n(a,m,nw) + accu2 * een_rescaled_n(num,a,m,nw) +&
                   accu3 * een_rescaled_single_n(a,m+l,nw) + accu4 *  een_rescaled_n(num,a,m+l,nw) + &
                   accu5 * een_rescaled_single_n_gl(kk,a,m,nw) + accu6 * een_rescaled_n_gl(num,kk,a,m,nw) + &
                   accu7 * een_rescaled_single_n_gl(kk,a,m+l,nw) + accu8 * een_rescaled_n_gl(num,kk,a,m+l,nw)

            delta_een_gl(num,kk,nw) = delta_een_gl(num,kk,nw) + accu * cn

            accu = accu1 * een_rescaled_single_n_gl(kk,a,m,nw) + accu2 * een_rescaled_n_gl(num,kk,a,m,nw) + &
                   accu3 * een_rescaled_single_n_gl(kk,a,m+l,nw) + accu4 * een_rescaled_n_gl(num,kk,a,m+l,nw)
            
            delta_een_gl(num,4,nw) = delta_een_gl(num,4,nw) + accu * cn * 2.d0

            accu1 =   een_rescaled_n_gl(i,kk,a,m+l,nw) * een_rescaled_single_e(i,k,nw) - accu1
            accu2 = - een_rescaled_n_gl(i,kk,a,m+l,nw) * een_rescaled_e(i,num,k,nw) - accu2
            accu3 =   een_rescaled_n_gl(i,kk,a,m,nw) * een_rescaled_single_e(i,k,nw) - accu3
            accu4 = - een_rescaled_n_gl(i,kk,a,m,nw) * een_rescaled_e(i,num,k,nw) - accu4
            ...
            accu1 = -een_rescaled_n_gl(i,kk,a,m+l,nw) * een_rescaled_single_e_gl(kk,i,k,nw) 
            accu2 = -een_rescaled_n_gl(i,kk,a,m+l,nw) * een_rescaled_e_gl(i,kk,num,k,nw)
            accu3 = -een_rescaled_n_gl(i,kk,a,m,nw) * een_rescaled_single_e_gl(kk,i,k,nw)
            accu4 = -een_rescaled_n_gl(i,kk,a,m,nw) * een_rescaled_e_gl(i,kk,num,k,nw)
            accu = accu1 * een_rescaled_single_n(a,m,nw) + accu2 * een_rescaled_n(num,a,m,nw) + &
                   accu3 * een_rescaled_single_n(a,m+l,nw) + accu4 *  een_rescaled_n(num,a,m+l,nw) 
        ...
        do  i = 1, elec_num
          accu1 = een_rescaled_n(i,a,m+l,nw) * een_rescaled_single_e_gl(4,i,k,nw) 
          accu2 = -een_rescaled_n(i,a,m+l,nw) * een_rescaled_e_gl(i,4,num,k,nw)
          accu3 = een_rescaled_n(i,a,m,nw) * een_rescaled_single_e_gl(4,i,k,nw)
          accu4 = -een_rescaled_n(i,a,m,nw) * een_rescaled_e_gl(i,4,num,k,nw)
          ...
          accu = accu1 * een_rescaled_single_n(a,m,nw) + accu2 * een_rescaled_n(num,a,m,nw) + &
                 accu3 * een_rescaled_single_n(a,m+l,nw) + accu4 *  een_rescaled_n(num,a,m+l,nw) + &
                 accu5 * een_rescaled_single_n_gl(4,a,m,nw) + accu6 * een_rescaled_n_gl(num,4,a,m,nw) + &
                 accu7 * een_rescaled_single_n_gl(4,a,m+l,nw) + accu8 * een_rescaled_n_gl(num,4,a,m+l,nw)

          delta_een_gl(num,4,nw) = delta_een_gl(num,4,nw) + accu * cn

          accu1 =   een_rescaled_n_gl(i,4,a,m+l,nw) * een_rescaled_single_e(i,k,nw) + accu1
          accu2 = - een_rescaled_n_gl(i,4,a,m+l,nw) * een_rescaled_e(i,num,k,nw) + accu2
          accu3 =   een_rescaled_n_gl(i,4,a,m,nw) * een_rescaled_single_e(i,k,nw) + accu3
          accu4 = - een_rescaled_n_gl(i,4,a,m,nw) * een_rescaled_e(i,num,k,nw) + accu4
    ...
    do n = 1, dim_c_vector
      ...
      do a = 1, nucl_num
        ...
        do kk = 1, 3
          ...
          en_ml  = een_rescaled_n(i,a,m+l,nw)
          en_m   = een_rescaled_n(i,a,m,nw)

          se_i   = een_rescaled_single_e(i,k,nw)
          e_i    = een_rescaled_e(i,num,k,nw)

          se_gl  = een_rescaled_single_e_gl(kk,i,k,nw)
          e_gl   = een_rescaled_e_gl(i,kk,num,k,nw)

          ! ---- first accumulation ----
          accu = en_ml *( se_gl*sn_m   + e_gl*n_num_m   + se_i*sn_gl_m   - e_i*n_gl_m ) &
               + en_m  *( se_gl*sn_ml  + e_gl*n_num_ml  + se_i*sn_gl_ml  - e_i*n_gl_ml )

          delta_een_g(num,kk,nw) = delta_een_g(num,kk,nw) + accu*cn

          ! ---- second accumulation ----
          nglp = een_rescaled_n_gl(i,kk,a,m+l,nw)
          ngm  = een_rescaled_n_gl(i,kk,a,m,nw)

          accu = ( nglp*se_i - en_ml *se_gl )*sn_m   &
               + (-nglp*e_i  - en_ml *e_gl )*n_num_m &
               + ( ngm *se_i - en_m  *se_gl )*sn_ml  &
               + (-ngm *e_i  - en_m  *e_gl )*n_num_ml

          delta_een_g(i,kk,nw) = delta_een_g(i,kk,nw) + accu*cn
          end do
        end do
      end do
    end do
                          ...
                          double* const delta_een_g )
{
  ...
  return qmckl_compute_jastrow_champ_factor_single_een_g_doc
    ...
    (context, num, walk_num, elec_num, nucl_num, cord_num, dim_c_vector,
     ...
     een_rescaled_single_n_gl, een_rescaled_e, een_rescaled_single_e, een_rescaled_e_gl, een_rescaled_single_e_gl, delta_een_g );
