...
integer function qmckl_compute_jastrow_champ_factor_single_een_doc( &
  ...
  do nw =1, walk_num
     ...
     #+begin_src f90 :comments org :tangle (eval f) :noweb yes
integer function qmckl_compute_jastrow_champ_factor_single_een_hpc( &
     context, num_in, walk_num, elec_num, nucl_num, cord_num,   &
     dim_c_vector, c_vector_full, lkpm_combined_index, &
     een_rescaled_n, een_rescaled_e, een_rescaled_single_n, &
     een_rescaled_single_e, delta_een) &
...
end function qmckl_compute_jastrow_champ_factor_single_een_hpc
     #+end_src

     #+begin_src f90 :comments org :tangle (eval f) :noweb yes
integer function qmckl_compute_jastrow_champ_factor_single_een_hpc2( &
     context, num_in, walk_num, elec_num, nucl_num, cord_num,   &
     dim_c_vector, c_vector_full, lkpm_combined_index, &
     een_rescaled_n, een_rescaled_e, een_rescaled_single_n, &
     een_rescaled_single_e, delta_een) &
  ...
  integer*8 :: i, a, j, l, k, p, m, n, nw, num
  double precision :: accu1, accu2, accu3, accu4, cn, eps
  integer*8        :: LDA, LDB, LDC
  integer*8, allocatable  :: non_zeros(:,:)
  ...
  do nw =1, walk_num
    do n = 1, dim_c_vector
      ...
      do a = 1, nucl_num
        ...
        if ( een_rescaled_single_n(a,m,nw) < eps .and. een_rescaled_n(num,a,m,nw) < eps ) cycle
        cn = c_vector_full(a, n)
        if(cn == 0.d0) cycle
        ...
        accu1 = accu1 * een_rescaled_single_n(a,m,nw) + accu2 * &
              een_rescaled_n(num,a,m,nw) + accu3 * een_rescaled_single_n(a,m+l,nw) + &
              accu4 * een_rescaled_n(num,a,m+l,nw)

        delta_een(nw) = delta_een(nw) + accu1 * cn
      end do
    end do
  end do

end function qmckl_compute_jastrow_champ_factor_single_een_hpc2
     #+end_src
...
qmckl_exit_code
qmckl_compute_jastrow_champ_factor_single_een_hpc2 (const qmckl_context context,
                              const int64_t num,
                              const int64_t walk_num,
                              const int64_t elec_num,
                              const int64_t nucl_num,
      ...
      do a = 1, nucl_num
        ...
        do kk = 1, 3
          do i = 1, elec_num
            ...
            accu = accu1 * een_rescaled_single_n(a,m,nw) + accu2 * een_rescaled_n(num,a,m,nw) + accu3 * een_rescaled_single_n(a,m+l,nw) + accu4 *  een_rescaled_n(num,a,m+l,nw) &
                + accu5 * een_rescaled_single_n_gl(kk,a,m,nw) + accu6 * een_rescaled_n_gl(num,kk,a,m,nw) + accu7 * een_rescaled_single_n_gl(kk,a,m+l,nw) + accu8 * een_rescaled_n_gl(num,kk,a,m+l,nw)

            delta_een_gl(num,kk,nw) = delta_een_gl(num,kk,nw) + accu * cn

            accu = accu1 * een_rescaled_single_n_gl(kk,a,m,nw) + accu2 * een_rescaled_n_gl(num,kk,a,m,nw) + accu3 * een_rescaled_single_n_gl(kk,a,m+l,nw) + accu4 * een_rescaled_n_gl(num,kk,a,m+l,nw)
        ...
        do  i = 1, elec_num
          ...
          accu = accu1 * een_rescaled_single_n(a,m,nw) + accu2 * een_rescaled_n(num,a,m,nw) + accu3 * een_rescaled_single_n(a,m+l,nw) + accu4 *  een_rescaled_n(num,a,m+l,nw) &
              + accu5 * een_rescaled_single_n_gl(4,a,m,nw) + accu6 * een_rescaled_n_gl(num,4,a,m,nw) + accu7 * een_rescaled_single_n_gl(4,a,m+l,nw) + accu8 * een_rescaled_n_gl(num,4,a,m+l,nw)
