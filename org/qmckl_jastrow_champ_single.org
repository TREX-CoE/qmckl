...
for (int i = 0; i < walk_num; i++) {
  printf("jastrow_een[%d] = %e\n", i, jastrow_een_new[i]-jastrow_een_old[i]);
  printf("delta_een[%d] = %e\n", i, delta_een[i]);
...
integer function qmckl_compute_een_rescaled_single_e_gl_f( &
  ...
  !integer, external :: qmckl_compute_een_rescaled_single_e_gl_testing_f

  !info = qmckl_compute_een_rescaled_single_e_gl_testing_f( &
  !   context, num_in, walk_num, elec_num, cord_num, rescale_factor_ee,  &
  !   coord, coord_ee, single_ee_distance, een_rescaled_single_e, een_rescaled_single_e_gl)
  !return 
  num = num_in + 1
  ...
  ! Prepare table of exponentiated distances raised to appropriate power
  print *, 'electron num', elec_num
  print *, 'single', coord(1), coord(2), coord(3)
  do nw = 1, walk_num
    do i = 1, elec_num
        print *, i, coord_ee(i,1,nw), coord_ee(i,2,nw), coord_ee(i,3,nw)
...
!   een_rescaled_single_e_gl = 0.0d0
    print *, 'cord_num', cord_num

    do l = 1, cord_num
      kappa_l = - dble(l) * rescale_factor_ee
      do i = 1, elec_num
          print *, kappa_l, elec_dist_gl(1, i), elec_dist_gl(2, i), elec_dist_gl(3, i)
 ...
 #+begin_src f90 :comments org :tangle (eval f) :noweb yes
integer function qmckl_compute_een_rescaled_single_e_gl_testing_f( &
     context, num_in, walk_num, elec_num, cord_num, rescale_factor_ee,  &
     coord, coord_ee, ee_distance, een_rescaled_e, een_rescaled_e_gl) &
     result(info)
  use qmckl
  implicit none
  integer(qmckl_context), intent(in)  :: context
  ...
  double precision      , intent(in)  :: coord(3)
  double precision      , intent(in)  :: coord_ee(elec_num,3,walk_num)
  double precision      , intent(in)  :: ee_distance(elec_num,walk_num)
  double precision      , intent(in)  :: een_rescaled_e(elec_num,0:cord_num,walk_num)
  double precision      , intent(out) :: een_rescaled_e_gl(elec_num,4,0:cord_num,walk_num)
  double precision,dimension(:,:,:),allocatable  :: elec_dist_gl
  double precision                    :: x, rij_inv, kappa_l
  integer*8                           :: i, j, k, l, nw, ii, num
  ...
  do nw = 1, walk_num
    j = num
      do i = 1, j-1
        rij_inv = 1.0d0 / ee_distance(i,  nw)
        do ii = 1, 3
          elec_dist_gl(i, ii, j) = (coord_ee(i, ii, nw) - coord(ii)) * rij_inv
        end do
...
!   een_rescaled_e_gl(:,:,:,0,nw) = 0.d0
    ...
    do l = 1, cord_num
      kappa_l = - dble(l) * rescale_factor_ee
        ...
        do i = 1, elec_num
          een_rescaled_e_gl(i, 1,  l, nw) = kappa_l * elec_dist_gl(i, 1, j)
          een_rescaled_e_gl(i, 2,  l, nw) = kappa_l * elec_dist_gl(i, 2, j)
          een_rescaled_e_gl(i, 3,  l, nw) = kappa_l * elec_dist_gl(i, 3, j)
          een_rescaled_e_gl(i, 4,  l, nw) = kappa_l * elec_dist_gl(i, 4, j)

          een_rescaled_e_gl(i, 4,  l, nw) = een_rescaled_e_gl(i, 4,  l, nw)              &
                    + een_rescaled_e_gl(i, 1,  l, nw) * een_rescaled_e_gl(i, 1,  l, nw)  &
                    + een_rescaled_e_gl(i, 2,  l, nw) * een_rescaled_e_gl(i, 2,  l, nw)  &
                    + een_rescaled_e_gl(i, 3,  l, nw) * een_rescaled_e_gl(i, 3,  l, nw)

          een_rescaled_e_gl(i,1,l,nw) = een_rescaled_e_gl(i,1,l,nw) * een_rescaled_e(i,l,nw)
          een_rescaled_e_gl(i,2,l,nw) = een_rescaled_e_gl(i,2,l,nw) * een_rescaled_e(i,l,nw)
          een_rescaled_e_gl(i,3,l,nw) = een_rescaled_e_gl(i,3,l,nw) * een_rescaled_e(i,l,nw)
          een_rescaled_e_gl(i,4,l,nw) = een_rescaled_e_gl(i,4,l,nw) * een_rescaled_e(i,l,nw)
        end do
...
double een_rescaled_ee_gl[walk_num][cord_num+1][elec_num][elec_num][4];
...
for (int nw = 0; nw < walk_num; nw++) {
    for (int i = 0; i < elec_num; i++) {
        for (int m = 0; m < 4; m++) {
          printf("een_rescaled_ee_gl[%d][%d][%d][%d][%d] = %e\n", nw, l, 2, m, i, een_rescaled_ee_gl[nw][l][2][m][i]);
          printf("een_rescaled_single_e_gl[%d][%d][%d][%d] = %e\n", nw, l, m,i, een_rescaled_single_e_gl[nw][l][m][i]);
