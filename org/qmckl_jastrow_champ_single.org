...
** Intermediate result for gradient and forces
                                 ...
                                 const int64_t size_max)
{
  ...
  int64_t sze = 4 * ctx->electron.walker.num * ctx->nucleus.num * (ctx->jastrow_champ.cord_num + 1) * (ctx->jastrow_champ.cord_num + 1);
  if (size_max < sze) {
    return qmckl_failwith( context,
                           QMCKL_INVALID_ARG_3,
                           "qmckl_get_jastrow_champ_single_tmp",
                           "Array too small. Expected 4 * ctx->electron.walker.num * ctx->nucleus.num * (ctx->jastrow_champ.cord_num + 1) * (ctx->jastrow_champ.cord_num + 1)");
  }
...
qmckl_exit_code qmckl_provide_jastrow_champ_single_tmp(qmckl_context context)
{
  ...
  if (ctx->single_point.date > ctx->single_point.tmp_date) {
    ...
    qmckl_memory_info_struct mem_info = qmckl_memory_info_struct_zero;
    mem_info.size = 4 * ctx->electron.walker.num * ctx->nucleus.num * (ctx->jastrow_champ.cord_num + 1) * (ctx->jastrow_champ.cord_num + 1) * sizeof(double);
...
*** Compute
     ...
     | ~een_rescaled_e~           | ~double[walk_num][0:cord_num][elec_num][elec_num]~                  | in     | Electron-electron rescaled distances                           |
     | ~een_rescaled_single_e~    | ~double[walk_num][0:cord_num][elec_num]~                            | in     | Electron-electron single rescaled distances                    |
     | ~tmp~                      | ~double[walk_num][0:cord_num][0:cord_num][nucl_num][4]~             | out    | Single tmp matrix                                              |
     |----------------------------+---------------------------------------------------------------------+--------+----------------------------------------------------------------|
...
integer(qmckl_exit_code) function qmckl_compute_jastrow_champ_factor_single_tmp_doc( &
  ...
  real(c_double)        , intent(in)  :: een_rescaled_e(elec_num, elec_num, 0:cord_num, walk_num)
  real(c_double)        , intent(in)  :: een_rescaled_single_e(elec_num, 0:cord_num, walk_num)
  real(c_double)        , intent(out) :: tmp(4, nucl_num, 0:cord_num, 0:cord_num, walk_num)
...
!
  !      do a = 1, nucl_num
  !        if(c_vector_full(a, n) == 0.d0) cycle
!
  !        do i = 1, elec_num
  !            tmp(1,a,m+l,k,nw) = tmp(1,a,m+l,k,nw) - een_rescaled_n(i,a,m+l,nw) * een_rescaled_single_e(i,k,nw)
  !            tmp(2,a,m+l,k,nw) = tmp(2,a,m+l,k,nw) + een_rescaled_n(i,a,m+l,nw) * een_rescaled_e(i,num,k,nw)
  !            tmp(3,a,m,k,nw) = tmp(3,a,m,k,nw) - een_rescaled_n(i,a,m,nw) * een_rescaled_single_e(i,k,nw)
  !            tmp(4,a,m,k,nw) = tmp(4,a,m,k,nw) + een_rescaled_n(i,a,m,nw) * een_rescaled_e(i,num,k,nw)
  !        end do
!
  !      end do
  !  end do
...
integer(qmckl_exit_code) function qmckl_compute_jastrow_champ_factor_single_een_gl_doc( &
  ...
  integer*8 :: i, a, j, l, k, p, m, n, nw, kk, num
  double precision :: accu, accu1, accu2, accu3, accu4, accu5, accu6, accu7, accu8, cn, eps
...
!               een_rescaled_n(num,a,m,nw) < eps ) cycle
           ...
           do kk = 1, 3
              do i = 1, elec_num
                 accu1 = een_rescaled_n(i,a,p,nw) * een_rescaled_single_e_gl(i,kk,k,nw) 
                 accu2 = een_rescaled_n(i,a,p,nw) * een_rescaled_e_gl(i,kk,num,k,nw)
                 ...
                 accu5 = een_rescaled_n(i,a,p,nw) * een_rescaled_single_e(i,k,nw) 
                 accu6 = - een_rescaled_n(i,a,p,nw) * een_rescaled_e(i,num,k,nw) 
                 ...
                 accu = accu1 * een_rescaled_single_n(a,m,nw) + accu2 * een_rescaled_n(num,a,m,nw) +&
                      accu3 * een_rescaled_single_n(a,p,nw) + accu4 *  een_rescaled_n(num,a,p,nw) + &
                      accu5 * een_rescaled_single_n_gl(kk,a,m,nw) + accu6 * een_rescaled_n_gl(num,kk,a,m,nw) + &
                      accu7 * een_rescaled_single_n_gl(kk,a,p,nw) + accu8 * een_rescaled_n_gl(num,kk,a,p,nw)
                 ...
                 accu = accu1 * een_rescaled_single_n_gl(kk,a,m,nw) + accu2 * een_rescaled_n_gl(num,kk,a,m,nw) + &
                      accu3 * een_rescaled_single_n_gl(kk,a,p,nw) + accu4 * een_rescaled_n_gl(num,kk,a,p,nw)

                 delta_een_gl(num,4,nw) = delta_een_gl(num,4,nw) + accu * cn * 2.d0

                 accu1 =   een_rescaled_n_gl(i,kk,a,p,nw) * een_rescaled_single_e(i,k,nw) - accu1
                 accu2 = - een_rescaled_n_gl(i,kk,a,p,nw) * een_rescaled_e(i,num,k,nw) - accu2
                 ...
                 accu1 = -een_rescaled_n_gl(i,kk,a,p,nw) * een_rescaled_single_e_gl(i,kk,k,nw) 
                 accu2 = -een_rescaled_n_gl(i,kk,a,p,nw) * een_rescaled_e_gl(i,kk,num,k,nw)
           ...
           do  i = 1, elec_num
              accu1 = een_rescaled_n(i,a,p,nw) * een_rescaled_single_e_gl(i,4,k,nw) 
              accu2 = -een_rescaled_n(i,a,p,nw) * een_rescaled_e_gl(i,4,num,k,nw)
              ...
              accu = accu1 * een_rescaled_single_n(a,m,nw) + accu2 * een_rescaled_n(num,a,m,nw) + &
                   ...
                   accu7 * een_rescaled_single_n_gl(4,a,p,nw) + accu8 * een_rescaled_n_gl(num,4,a,p,nw)

              delta_een_gl(num,4,nw) = delta_een_gl(num,4,nw) + accu * cn

              accu1 =   een_rescaled_n_gl(i,4,a,p,nw) * een_rescaled_single_e(i,k,nw) + accu1
              accu2 = - een_rescaled_n_gl(i,4,a,p,nw) * een_rescaled_e(i,num,k,nw) + accu2
...
*** Compute
     ...
     | ~een_rescaled_e_gl~        | ~double[walk_num][0:cord_num][elec_num][4][elec_num]~               | in     | Electron-electron rescaled distances derivatives               |
     | ~een_rescaled_single_e_gl~ | ~double[walk_num][0:cord_num][4][elec_num]~                         | in     | Electron-electron single rescaled distances derivatives        |
     | ~tmp~                      | ~double[walk_num][0:cord_num][0:cord_num][nucl_num][4]~             | in     | Temporary storage for intermediate results                     |
...
integer(qmckl_exit_code) function qmckl_compute_jastrow_champ_factor_single_een_g_doc( &
  ...
  real(c_double)        , intent(in)  :: een_rescaled_e_gl(elec_num, 4, elec_num, 0:cord_num, walk_num)
  real(c_double)        , intent(in)  :: een_rescaled_single_e_gl(elec_num, 4, 0:cord_num, walk_num)
  real(c_double)        , intent(in)  :: tmp(4, nucl_num, 0:cord_num, 0:cord_num, walk_num)
    ...
    do n = 1, dim_c_vector
      ...
      do a = 1, nucl_num
        ...
        do kk = 1, 3
          ...
          accu = accu1*sn_m   + accu2*n_num_m   - tmp(1,a,m+l,k,nw)*sn_gl_m   - tmp(2,a,m+l,k,nw)*n_gl_m &
               + accu5*sn_ml  + accu6*n_num_ml  - tmp(3,a,m,k,nw)*sn_gl_ml    - tmp(4,a,m,k,nw)*n_gl_ml
