...
integer function qmckl_compute_jastrow_champ_delta_p_doc( &
  ...
  do nw=1, walk_num
     ...
     #+begin_src f90 :comments org :tangle (eval f) :noweb yes
integer function qmckl_compute_jastrow_champ_delta_p_hpc2( &
     context, num_in, walk_num, elec_num, nucl_num, cord_num,   &
     een_rescaled_n, een_rescaled_e, een_rescaled_single_n, een_rescaled_single_e, delta_p) &
     result(info) bind(C)
  use, intrinsic :: iso_c_binding
    ...
    do i=1, cord_num-1
      ...
      do c=1,cord_num-i
          do a=1,nucl_num
            ...
            dn = 0.0d0
            do j = 1, elec_num
              dn = dn + een_rescaled_delta_e(j) * een_rescaled_n(j,a,c,nw)
            end do
            delta_p(num,a,c,i,nw) = delta_p(num,a,c,i,nw) + dn
...
end function qmckl_compute_jastrow_champ_delta_p_hpc2
     #+end_src
...
#pragma omp simd
      ...
      /* dn = sum_j een_rescaled_delta_e(j) */
      double dn_sum = 0.0;
#ifdef HAVE_OPENMP
#pragma omp simd
#endif
...
#pragma omp simd
      ...
      /* delta_p(num,a,0,i,nw) += dn_sum for all nuclei a */
      for (int64_t a = 0; a < D_nucl; ++a) {
...
//      const int32_t m4 = elec_num;
//      const int32_t n4 = nucl_num;
//      const int32_t incx = 1;
//      const int32_t incy = elec_num;
//      const double alpha = 1.0;
//      const char t = 'T';
//      dgemv(&t,  &m4, &n4, &alpha, &(EEN_N(0,0,c,nw)), &m4, 
//            een_rescaled_delta_e, &incx, &alpha, &(DELTA_P(num_idx,0,c,i,nw)), &incy);
        qmckl_dgemv(context, 'T',  elec_num, nucl_num, 1.0, &(EEN_N(0,0,c,nw)), elec_num,
                    een_rescaled_delta_e, 1, 1.0, &(DELTA_P(num_idx,0,c,i,nw)), elec_num);
  ...
  do nw =1, walk_num
     ...
     do n = 1, dim_c_vector
        ...
        do a = 1, nucl_num
           ...
           if (m == 0) then
              do j = 1, elec_num
                 accu = accu + delta_p(j,a,m+l,k,nw)
              end do
           else
