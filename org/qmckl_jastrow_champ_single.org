...
#pragma omp simd
      ...
      for (int64_t c = 0; c <= cord_num - i; ++c) {
        for (int64_t a = 0; a < D_nucl; ++a) {
...
#pragma omp simd
      ...
      /*
          const int elec_num32 = elec_num;
          const int nucl_num32 = nucl_num;
          const int one = 1;
          const double* delta_p_column = &DELTA_P(0, a, c, i, nw);
          dger(&elec_num32, &nucl_num32, &dn,
               een_rescaled_delta_e, &one,
               delta_p_column, &one,
               delta_p_matrix, &elec_num);
    ,*/
  }
