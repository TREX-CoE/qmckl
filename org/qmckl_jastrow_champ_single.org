...
#pragma omp simd
          ...
          /* dn_sum = sum_j een_rescaled_delta_e(j) * een_rescaled_n(j,a,c,nw) */
          double dn_sum = 0.0;
#ifdef HAVE_OPENMP
#pragma omp simd reduction(+:dn_sum)
#endif
          for (int64_t j = 0; j < D_elec; ++j) {
            dn_sum += een_rescaled_delta_e[j] * EEN_N(j, a, c, nw);
          }

          /* delta_p(num,a,c,i,nw) += dn_sum */
          DELTA_P(num_idx, a, c, i, nw) += dn_sum;
        }
      }
    }
  }
