...
particles in quantum chemistry calculations. Inter-particle distances are
fundamental quantities needed throughout QMC simulations, appearing in the
calculation of wavefunctions, local energies, Jastrow factors, and various
other quantities.

The distance functions are designed for performance and flexibility, supporting
different memory layouts (normal and transposed) to accommodate various usage
patterns and enable efficient vectorization. These functions form a critical
computational kernel that is called frequently during QMC simulations.
