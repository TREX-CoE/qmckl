...
In quantum Monte Carlo simulations, electrons are treated quantum mechanically
while their positions are sampled stochastically. The electron data structure
stores essential information about the electronic system, including the number
of electrons and their spatial positions.

In conventional QMC simulations, up-spin and down-spin electrons are
treated as distinguishable particles due to the Pauli exclusion principle.
The ~electron~ data structure contains the number of up-spin and down-spin 
electrons separately, along with their coordinates for multiple walkers 
(different electronic configurations sampled in the Monte Carlo process).

The electrons are stored as points in the following order: for each walker,
first up-spin electrons and then down-spin electrons. This ordering convention
is maintained throughout the library for consistency.

** Memory layout

If the points are set with the ='N'= flag, the order of
the components is =[ (x,y,z), (x,y,z), ... ]= (normal or row-major order).
Using the ='T'= flag, it is =[ (x,x,x,...), (y,y,y,...), (z,z,z,...) ]= 
(transposed or column-major order). The choice of memory layout can affect
cache performance and vectorization efficiency.
...
* Context
  ...
  potential energies) in the context. The walker mechanism allows multiple
  electronic configurations to be handled simultaneously, which is essential
  for quantum Monte Carlo calculations.
...
<<<<<<< HEAD
=======
>>>>>>> a1f7840d6a4de856d1dcefa7b916ffc6aad04d8c
** Deep copy
