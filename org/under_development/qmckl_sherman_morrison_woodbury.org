...
This module implements efficient algorithms for updating matrix inverses using
the Sherman-Morrison and Woodbury matrix inversion formulas. These mathematical
techniques are crucial for quantum Monte Carlo calculations, where the inverse
of the Slater matrix needs to be updated frequently as electrons move.

The Sherman-Morrison formula allows efficient rank-1 updates to a matrix inverse,
while the Woodbury formula extends this to rank-k updates. These methods avoid
the computational cost of full matrix inversion (O(n³)) by performing updates
in O(n²) time. This module provides both low-level and high-level functions
that implement these formulas to update the inverse of a non-singular matrix
...
~qmckl_exit_code qmckl_sm_naive_{Dim}~ is a C function-template that is used to generate instances of C functions based on the range given above. The advantage of this method is that for each of these instances all the dimensions and loop-bounds are known at compile time, allowing the compiler to optimize more aggressively.
...
#+begin_src c :tangle (eval c) :comments org :noweb yes :exports none
...
#+begin_src c :tangle (eval c) :comment org :exports none
qmckl_exit_code qmckl_sm_splitting(
    const qmckl_context context,
    const uint64_t LDS,
    const uint64_t Dim,
