...
integer function qmckl_dgemm_f(context, TransA, TransB, &
  ...
  res = qmckl_dgemm_tiled(context, m, n, k, A, LDA, B, LDB, C, LDC)
  print *,"m=",m," n=",n," k=",k
  print *,"A="
  do i=1,m
    do j=1,k
...
integer(qmckl_exit_code) function test_qmckl_dgemm(context) bind(C)
  ...
  allocate( A(LDA,m), B(LDB,k) , C(LDC,n), D(LDC,n))
  ...
  do j=1,n
     do i=1,m
        do l=1,k
           D(i,j) = D(i,j) + A(l,i)*B(j,l)
        end do
        x = x + (D(i,j) - C(i,j))**2
        print *," D=",D(i,j)," C=",C(i,j)
     end do
  end do
  print *,"x = ",dabs(x)
...
A = np.array([[ 1.000  , 5.000  , 9.000  , 2.000 ],
              [ 6.000  , 10.000 ,  3.000 ,  7.000 ],
              [ 11.000 ,  4.000 ,  8.000 ,  12.000 ]])

B = np.array([[  1.000 , 5.000  , 9.000  , 10.000 ,  -2.000], 
              [ -6.000 , 10.000 ,  11.000,   3.000,   7.000], 
              [ 11.000 , 12.000 ,  4.000 ,  8.000 ,  12.000], 
              [ 15.000 , 5.000  , 9.000  , 13.000 ,  14.000]]) 
    ...
    #+begin_src c :comments link :tangle (eval c_test) :exports none
{
  ...
  //                 59., 141., 223. };
   ...
   double c[15] =  { 100. , 173. , 118. , 123. , 169.,
                      84. , 201. , 239. , 205. , 192.,
                     255. , 251. , 283. , 342. , 270. };
  double cnew[15];

  qmckl_exit_code rc;
  qmckl_matrix A = qmckl_matrix_alloc(context, 4, 3);
  rc = qmckl_matrix_of_double(context, a, 12, &A);
  assert(rc == QMCKL_SUCCESS);

  qmckl_matrix B = qmckl_matrix_alloc(context, 5, 4);
  rc = qmckl_matrix_of_double(context, b, 20, &B);
  assert(rc == QMCKL_SUCCESS);

  qmckl_matrix C = qmckl_matrix_alloc(context, 3, 5);
  rc = qmckl_matmul(context, 'T', 'T', 1.0, A, B, 0., &C);
  assert(rc == QMCKL_SUCCESS);
