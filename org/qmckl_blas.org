...
qmckl_dgemm_sparse_b_nn(const qmckl_context context,
                        ...
                        const double a,
                        const double *A, const int64_t lda,
                        const CSRMatrix *Bcsr, const double b,
                        double *C, const int64_t ldc)
{
  /* Basic context / arg checks (replace error codes/messages as desired) */
  if (qmckl_context_check(context) == 0) {
  ...
  if (Bcsr->density < threshold) {
    ...
    for (int64_t kk=0 ; kk<k ; ++kk) {
      const int64_t brow_start = Bcsr->row_ptr[kk];
      const int64_t brow_end   = Bcsr->row_ptr[kk+1];
      const double* a = &(A[kk*lda]);

      for (int64_t idxB = brow_start ; idxB < brow_end ; ++idxB) {
        const int64_t j = Bcsr->col_idx[idxB];
        const double valB = Bcsr->val[idxB];
...
#pragma omp simd
    ...
    qmckl_dgemm(context, 'N', 'N', m, n, k, a, A, lda, Bcsr->A, Bcsr->lda, b, C, ldc);
