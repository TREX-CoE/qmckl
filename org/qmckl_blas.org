...
** ~qmckl_dgemm_sparse_nn~
    ...
    #+begin_src c :tangle (eval h_private_type) :comments org :exports none
typedef struct {
...
qmckl_dgemm_sparse_b_nn(const qmckl_context context,
                        ...
                        const double a,
                        const double *A, const int64_t lda,
                        const CSCMatrix *Bcsc, const double b,
                        double *C, const int64_t ldc)
{
  /* Basic context / arg checks (replace error codes/messages as desired) */
  if (qmckl_context_check(context) == 0) {
...
#endif
        for (int64_t i=0 ; i<m ; ++i) {
          C[i+j*ldc] += A[i+kk1*lda] * valB1 + A[i+kk2*lda] * valB2 +
                        A[i+kk3*lda] * valB3 + A[i+kk4*lda] * valB4 ;
        }
      }
...
** ~qmckl_dgemv~
   ...
   | ~A~       | ~double[][lda]~ | in     | Array containing the matrix $A$       |
   | ~lda~     | ~int64_t~       | in     | Leading dimension of array ~A~        |
   | ~X~       | ~double[]~      | in     | Array containing the matrix $B$       |
   | ~incx~    | ~int64_t~       | in     | Leading dimension of array ~B~        |
   | ~beta~    | ~double~        | in     | \beta                                 |
   | ~y~       | ~double[]~      | out    | Array containing the matrix $C$       |
   | ~incy~    | ~int64_t~       | in     | Leading dimension of array ~C~        |
