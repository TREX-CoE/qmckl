...
The local energy is the fundamental quantity computed in quantum Monte Carlo
methods. It is the expectation value of the Hamiltonian operator for a given
electronic configuration, and its statistical average over many Monte Carlo
samples converges to the true ground state energy of the system.

In this module, we calculate the local energy $E_L$ as the sum of the 
...
The local energy should ideally be constant everywhere in space for the exact
wavefunction. Variations in the local energy indicate the quality of the
trial wavefunction, making it an important diagnostic tool as well as the
primary computed quantity.

** Kinetic Energy

Where the kinetic energy is computed from the Laplacian of the wavefunction:

\[
KE = -\frac{1}{2}\frac{\bigtriangleup \Psi}{\Psi}
\]

The Laplacian of the wavefunction measures the local curvature of the 
wavefunction. In the single-determinant case, it can be efficiently computed
using the inverse of the Slater matrix:
...
The potential energy comprises all electrostatic interactions in the system:

\[
PE = \mathcal{V}_{ee} + \mathcal{V}_{en} + \mathcal{V}_{nn}
\]

These three terms represent electron-electron repulsion, electron-nucleus
attraction, and nucleus-nucleus repulsion, respectively. The potential energy
is calculated as the sum of single-electron contributions for the
electron-dependent terms:
...
** Monte Carlo sampling quantities

The remaining quantities that are required for performing
a single Metropolis Monte Carlo step in variational Monte Carlo are as follows:

1. Drift Vector - $F(x)$: The quantum force that guides the diffusion process

\[
F(x) = 2\frac{\nabla \Psi(r)}{\Psi(r)}
\]

2. Diffusion move - $y$: The proposed new electron configuration

\[
y = x + D F(x) \delta t + \chi
\]

Where $\chi$ is a random number with Gaussian distribution centered at zero
and standard deviation of $\sqrt{2D\delta t}$. Here $D$ is the diffusion coefficient 
and $\delta t$ is the time step.

3. Acceptance probability - $\min\left[1, q(y,x)\right]$: The Metropolis acceptance criterion

\[
q(y,x) = Exp\left[\frac{\delta r}{2} ( F(x) + F(y)) - \frac{D\delta t}{4}\left( F(x)^2 - F(y)^2\right)\right]
\]

With these quantities, a single-determinant variational Monte Carlo (VMC) 
simulation can be carried out. The combination of the drift vector (quantum force),
diffusion process, and Metropolis acceptance criterion implements importance
sampling, which dramatically improves the efficiency of the QMC calculation
by sampling regions of configuration space where the wavefunction is large.
...
integer function qmckl_compute_kinetic_energy_f(context, walk_num, &
  ...
  do iwalk = 1, walk_num
   ...
   #+end_src
    ...
    #+CALL: generate_private_c_interface(table=qmckl_compute_kinetic_energy_args,rettyp=get_value("CRetType"),fname="qmckl_compute_kinetic_energy"))
...
integer function qmckl_compute_potential_energy_f(context, walk_num, &
  ...
  do iwalk = 1, walk_num
   ...
   #+end_src
    ...
    #+CALL: generate_private_c_interface(table=qmckl_compute_potential_energy_args,rettyp=get_value("CRetType"),fname="qmckl_compute_potential_energy"))
...
integer function qmckl_compute_local_energy_f(context, walk_num, &
  ...
  do iwalk = 1, walk_num
   ...
   #+CALL: generate_private_c_interface(table=qmckl_compute_local_energy_args,rettyp=get_value("CRetType"),fname="qmckl_compute_local_energy"))
...
integer function qmckl_compute_drift_vector_f(context, walk_num, &
  ...
  do iwalk = 1, walk_num
   ...
   #+CALL: generate_private_c_interface(table=qmckl_compute_drift_vector_args,rettyp=get_value("CRetType"),fname="qmckl_compute_drift_vector"))
