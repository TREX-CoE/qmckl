...
QMCkl provides custom memory management functions to enable fine-grained
control over memory allocation and deallocation. This design allows optimized 
implementations of the library to override the default allocation strategy to 
take advantage of specialized memory allocators, aligned allocations for 
vectorization, pinned memory for GPU transfers, or other hardware-specific 
optimizations.

By centralizing memory allocation through the context, the library can:
- Track all allocations for proper cleanup when the context is destroyed
- Implement custom allocation strategies (e.g., memory pools, aligned allocations)
- Provide detailed memory usage statistics for profiling
- Ensure thread-safe memory operations when needed

The memory management API is designed to be simple and consistent with
standard C memory functions, while providing the flexibility needed for
...
* Memory data structure for the context
  ...
  The QMCkl library maintains detailed information about all memory allocations
  made through the context. This tracking enables automatic cleanup when the
  context is destroyed and provides visibility into memory usage patterns.
  ...
  This metadata structure can be extended in specialized implementations to
  support additional allocation strategies or memory types without changing
  the public API.
...
** Memory allocation
  ...
  The context is passed to the function so that the library can store data 
  related to the allocation inside the context. In this particular implementation 
  of the library, we maintain a registry of allocated pointers so that all memory 
  can be properly freed when the library is de-initialized. This prevents memory 
  leaks and ensures clean shutdown.
...
* End of files                                                     :noexport:
  ...
  #+end_src
** Test
  #+begin_src c :comments org :tangle (eval c_test) :exports none
  return 0;
}
