  ...
  for (int64_t nw=0; nw < walk_num; ++nw) {
    ...
    { int64_t i=1; 
      for (int64_t j=1; j<=cord_num-i; ++j){
          qmckl_dgemm(context, 'N', 'N', M, N, K, alpha,
                      &(een_rescaled_e[af*(i+nw*(cord_num+1))]), LDA,
                      &(een_rescaled_n[bf*(j+nw*(cord_num+1))]), LDB, beta,
    ...
    */
    ...
    CSRMatrix *ee[cord_num];
    double eps = 1.e-8;
    for (int64_t i=1; i<cord_num; ++i){
      ee[i] = qmckl_dense_to_csr(M, K,  &(een_rescaled_e[af*(i+nw*(cord_num+1))]), LDA, eps);
    }
      
    CSRMatrix *eN[cord_num];
    for (int64_t j=1; j<cord_num; ++j){
      eN[j] = qmckl_dense_to_csr(K, N, &(een_rescaled_n[bf*(j+nw*(cord_num+1))]), LDB, eps);
    }
    
    for (int64_t i=1; i<cord_num; ++i){
      for (int64_t j=1; j<=cord_num-i; ++j){
          qmckl_dgemm_sparse_nn(context, M, N, K, alpha,
                                ee[i], eN[j], beta,
                                &(tmp_c[cf*(j+(cord_num+1)*(i+nw*cord_num))]), LDC);
        /*
          qmckl_dgemm(context, 'N', 'N', M, N, K, alpha,
    ...
    for (int64_t i=2; i<cord_num; ++i){
      free_csr(ee[i]);
    }
    for (int64_t j=2; j<cord_num; ++j){
      free_csr(eN[j]);
    }
  }
