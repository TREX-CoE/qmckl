...
**** Compute
     ...
     #+begin_src f90 :comments org :tangle (eval f) :noweb yes
integer(qmckl_exit_code) function qmckl_compute_jastrow_champ_factor_een_rescaled_e_gl_doc( &
     context, walk_num, elec_num, cord_num, rescale_factor_ee,  &
...
#endif
    for (size_t nw = 0; nw < (size_t) walk_num; ++nw) {
      memset(&factor_een_grad[elec_num*3*nw], 0, elec_num*3*sizeof(double));
    }
    return QMCKL_SUCCESS;
  }
...
for (int nw=0 ; nw<walk_num ; nw++) {
  for (int k=0 ; k<3; k++) {
    for (int i=0 ; i<elec_num ; i++) {
      assert(fabs(factor_een_gl[nw][k][i] - factor_een_grad[nw][k][i]) < 1e-12);
    }
  }
 }
     #+end_src
...
interface
   integer(qmckl_exit_code) function qmckl_get_jastrow_champ_grad (context, &
        grad, size_max) bind(C)
     use, intrinsic :: iso_c_binding
     import
     implicit none
     integer (qmckl_context) , intent(in), value :: context
     integer(c_int64_t), intent(in), value       :: size_max
     real(c_double),   intent(out)               :: grad(size_max)
   end function qmckl_get_jastrow_champ_grad
end interface
 #+end_src
...
for (int64_t k=0 ; k< walk_num ; ++k) {
  for (int64_t m=0 ; m<3; ++m) {
    for (int64_t e=0 ; e<elec_num; ++e) {
      assert (total_j_deriv[k][m][e] - total_j_grad[k][m][e] < 1.e-12);
    }
  }
 }
