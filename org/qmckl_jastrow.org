...
<<pre2>>
  ...
  if (ctx->jastrow.a_vector != NULL) {
    rc = qmckl_free(context, ctx->jastrow.a_vector);
    if (rc != QMCKL_SUCCESS) {
      return qmckl_failwith( context, rc,
                             "qmckl_set_jastrow_a_vector",
...
<<pre2>>
  ...
  if (ctx->jastrow.b_vector != NULL) {
    rc = qmckl_free(context, ctx->jastrow.b_vector);
    if (rc != QMCKL_SUCCESS) {
      return qmckl_failwith( context, rc,
                             "qmckl_set_jastrow_b_vector",
...
<<pre2>>
  ...
  qmckl_exit_code rc = qmckl_provide_dim_c_vector(context);
  if (rc != QMCKL_SUCCESS) return rc;

  int64_t dim_c_vector;
  rc = qmckl_get_jastrow_dim_c_vector(context, &dim_c_vector);
  if (rc != QMCKL_SUCCESS) return rc;
  ...
  if (ctx->jastrow.c_vector != NULL) {
    rc = qmckl_free(context, ctx->jastrow.c_vector);
    if (rc != QMCKL_SUCCESS) {
      return qmckl_failwith( context, rc,
                             "qmckl_set_jastrow_c_vector",
...
** Electron-electron-nucleus rescaled distances for each order
...
** Electron-electron-nucleus rescaled distances for each order and derivatives
...
qmckl_exit_code qmckl_get_jastrow_dim_c_vector(qmckl_context context, int64_t* const dim_c_vector)
{
  ...
  rc = qmckl_provide_dim_c_vector(context);
  if (rc != QMCKL_SUCCESS) return rc;
...
qmckl_exit_code qmckl_get_jastrow_c_vector_full(qmckl_context context, double* const c_vector_full)
{
  ...
  rc = qmckl_provide_c_vector_full(context);
  if (rc != QMCKL_SUCCESS) return rc;
...
*** Provide                                                        :noexport:
    ...
    #+begin_src c :comments org :tangle (eval h_private_func) :noweb yes :exports none
qmckl_exit_code qmckl_provide_dim_c_vector(qmckl_context context);
qmckl_exit_code qmckl_provide_c_vector_full(qmckl_context context);
...
qmckl_exit_code qmckl_provide_dim_c_vector(qmckl_context context)
{
  ...
  /* Compute if necessary */
  if (ctx->date > ctx->jastrow.dim_c_vector_date) {

    qmckl_exit_code rc =
      qmckl_compute_dim_c_vector(context,
                                  ctx->jastrow.cord_num,
...
qmckl_exit_code qmckl_provide_c_vector_full(qmckl_context context)
{
  ...
  /* Check if dim_c_vector is provided */
  qmckl_exit_code rc = qmckl_provide_dim_c_vector(context);
  if(rc != QMCKL_SUCCESS) return rc;
