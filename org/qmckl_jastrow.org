...
* Context
  ...
  computed data:
    ...
    | ~tmp_c~                       | ~double[walk_num][elec_num][nucl_num][ncord + 1][ncord]~     | vector of non-zero coefficients                                                                         |             |
    | ~dtmp_c~                      | ~double[walk_num][elec_num][4][nucl_num][ncord + 1][ncord]~  | vector of non-zero coefficients                                                                         |             |
...
*** Compute tmp_c
   ...
   :END:
    ...
    | ~tmp_c~          | ~double[walk_num][elec_num][nucl_num][0:cord_num][0:cord_num-1]~ | out    | vector of non-zero coefficients   |
...
integer function qmckl_compute_tmp_c_f(context, cord_num, elec_num, nucl_num, &
  ...
  double precision      , intent(in)  :: een_rescaled_e(0:cord_num, elec_num, elec_num, walk_num)
  double precision      , intent(in)  :: een_rescaled_n(0:cord_num, nucl_num, elec_num, walk_num)
  double precision      , intent(out) :: tmp_c(0:cord_num-1, 0:cord_num, nucl_num, elec_num, walk_num)
  double precision                    :: x
  integer*8                           :: i, a, l, kk, p, lmax, nw
  ...
  info = qmckl_dgemm(context,TransA, TransB, M, N, K, alpha,     &
                     een_rescaled_e(i,1,1,nw),LDA*1_8,                     &
                     een_rescaled_n(0,1,1,nw),LDB*1_8,                     &
                     beta,                                       &
                     tmp_c(0,i,1,1,nw),LDC)
  end do
  end do
	...
	bind(C) result(info)
      ...
      real    (c_double ) , intent(in)          :: een_rescaled_e(0:cord_num,elec_num,elec_num,walk_num)
      real    (c_double ) , intent(in)          :: een_rescaled_n(0:cord_num,nucl_num,elec_num,walk_num)
      real    (c_double ) , intent(out)         :: tmp_c(0:cord_num-1,0:cord_num,nucl_num,elec_num,walk_num)
...
*** Compute dtmp_c
   ...
   :END:
    ...
    | ~een_rescaled_n~         | ~double[walk_num][elec_num][nucl_num][0:cord_num]~               | in     | Electron-nucleus rescaled factor              |
    | ~dtmp_c~                 | ~double[walk_num][elec_num][nucl_num][0:cord_num][0:cord_num-1]~ | out    | vector of non-zero coefficients               |
...
integer function qmckl_compute_dtmp_c_f(context, cord_num, elec_num, nucl_num, &
  ...
  double precision      , intent(in)  :: een_rescaled_n(0:cord_num, nucl_num, elec_num, walk_num)
  double precision      , intent(out) :: dtmp_c(0:cord_num-1, 0:cord_num, nucl_num, 4, elec_num, walk_num)
  double precision                    :: x
  integer*8                           :: i, a, l, kk, p, lmax, nw
  character                           :: TransA, TransB
  double precision                    :: alpha, beta
  ...
  N = nucl_num*(cord_num + 1)
  K = elec_num
  LDA = size(een_rescaled_e_deriv_e,1)
  LDB = size(een_rescaled_n,1)
  LDC = size(dtmp_c,1)
  ...
  do i=0, cord_num-1
  info = qmckl_dgemm(context,TransA, TransB, M, N, K, alpha,     &
                     een_rescaled_e_deriv_e(i,1,1,1,nw),LDA*1_8,                     &
                     een_rescaled_n(0,1,1,nw),LDB*1_8,                     &
                     beta,                                       &
                     dtmp_c(0,i,1,1,1,nw),LDC)
  end do
  end do
	...
	bind(C) result(info)
      ...
      real    (c_double ) , intent(in)          :: een_rescaled_n(0:cord_num,nucl_num,elec_num,walk_num)
      real    (c_double ) , intent(out)         :: dtmp_c(0:cord_num-1,0:cord_num,nucl_num,elec_num,walk_num)
...
qmckl_exit_code qmckl_provide_factor_een(qmckl_context context)
{
  ...
  if (ctx->date > ctx->jastrow.factor_een_date) {
    ...
    qmckl_exit_code rc =
      qmckl_compute_factor_een_naive(context,
                              ctx->electron.walk_num,
                              ctx->electron.num,
                              ctx->nucleus.num,
                              ctx->jastrow.cord_num,
...
integer function qmckl_compute_factor_een_f(context, walk_num, elec_num, nucl_num, cord_num,   &
  ...
  integer*8             , intent(in)  :: lkpm_combined_index(dim_cord_vect,4)
  double precision      , intent(in)  :: cord_vect_full(nucl_num, dim_cord_vect)
  double precision      , intent(in)  :: tmp_c(0:cord_num-1, 0:cord_num, elec_num, elec_num, walk_num)
  double precision      , intent(in)  :: een_rescaled_n(0:cord_num, nucl_num, elec_num, walk_num)
  double precision      , intent(out) :: factor_een(walk_num)
  ...
  do n = 1, dim_cord_vect
    ...
    do a = 1, nucl_num
      ...
      accu = 0.0d0
      do j = 1, elec_num
        accu = accu + een_rescaled_n(m,a,j,nw) * tmp_c(k,m+l,a,j,nw)
      end do
...
qmckl_exit_code qmckl_provide_factor_een_deriv_e(qmckl_context context)
{
  ...
  if (ctx->date > ctx->jastrow.factor_een_deriv_e_date) {
    ...
    qmckl_exit_code rc =
      qmckl_compute_factor_een_deriv_e_naive(context,
                              ctx->electron.walk_num,
                              ctx->electron.num,
                              ctx->nucleus.num,
                              ctx->jastrow.cord_num,
...
*** Compute
   ...
   :END:
    ...
    | ~cord_vect_full~         | ~double[dim_cord_vect][nucl_num]~                                 | in     | full coefficient vector                         |
    | ~lkpm_combined_index~    | ~int64_t[4][dim_cord_vect]~                                       | in     | combined indices                                |
    | ~tmp_c~                  | ~double[walk_num][elec_num][nucl_num][cord_num + 1][cord_num]~    | in     | Temporary intermediate tensor                   |
    | ~dtmp_c~                 | ~double[walk_num][elec_num][4][nucl_num][cord_num + 1][cord_num]~ | in     | Derivative of the temporary intermediate tensor |
    | ~een_rescaled_n~         | ~double[walk_num][elec_num][nucl_num][0:cord_num]~                | in     | Electron-nucleus rescaled factor                |
...
integer function qmckl_compute_factor_een_deriv_e_f(context, walk_num, elec_num, nucl_num, cord_num, dim_cord_vect,            &
  ...
  double precision      , intent(in)  :: cord_vect_full(nucl_num, dim_cord_vect)
  double precision      , intent(in)  :: tmp_c(0:cord_num-1, 0:cord_num, elec_num, elec_num, walk_num)
  double precision      , intent(in)  :: dtmp_c(0:cord_num-1, 0:cord_num, elec_num, 4, elec_num, walk_num)
  double precision      , intent(in)  :: een_rescaled_n(0:cord_num, nucl_num, elec_num, walk_num)
  double precision      , intent(in)  :: een_rescaled_n_deriv_e(0:cord_num, nucl_num, 4, elec_num, walk_num)
  double precision      , intent(out) :: factor_een_deriv_e(elec_num,4,walk_num)
  ...
  do n = 1, dim_cord_vect
    ...
    do a = 1, nucl_num
      ...
      do ii = 1, 4
        do j = 1, elec_num
          factor_een_deriv_e(j,ii,nw) = factor_een_deriv_e(j,ii,nw) +                           (&
                                        tmp_c(k,m,a,j,nw)       * een_rescaled_n_deriv_e(m+l,a,ii,j,nw) + &
                                        dtmp_c(k,m,a,ii,j,nw)   * een_rescaled_n(m+l,a,j,nw)            + &
                                        dtmp_c(k,m+l,a,ii,j,nw) * een_rescaled_n(m,a,j,nw)              + &
                                        tmp_c(k,m+l,a,j,nw)     * een_rescaled_n_deriv_e(m,a,ii,j,nw)     &
                                        ) * cn
        end do
      end do
      ...
      do j = 1, elec_num
        factor_een_deriv_e(j,4,nw) = factor_een_deriv_e(j,4,nw) +                              (&
                                      dtmp_c(k,m  ,a,1,j,nw) * een_rescaled_n_deriv_e(m+l,a,1,j,nw)  + &
                                      dtmp_c(k,m  ,a,2,j,nw) * een_rescaled_n_deriv_e(m+l,a,2,j,nw)  + &
                                      dtmp_c(k,m  ,a,3,j,nw) * een_rescaled_n_deriv_e(m+l,a,3,j,nw)  + &
                                      dtmp_c(k,m+l,a,1,j,nw) * een_rescaled_n_deriv_e(m  ,a,1,j,nw)  + &
                                      dtmp_c(k,m+l,a,2,j,nw) * een_rescaled_n_deriv_e(m  ,a,2,j,nw)  + &
                                      dtmp_c(k,m+l,a,3,j,nw) * een_rescaled_n_deriv_e(m  ,a,3,j,nw)    &
                                      ) * cn
      end do
    end do
  end do
	...
	bind(C) result(info)
      ...
      real    (c_double ) , intent(in)          :: cord_vect_full(nucl_num,dim_cord_vect)
      integer (c_int64_t) , intent(in)          :: lkpm_combined_index(dim_cord_vect,4)
      real    (c_double ) , intent(in)          :: tmp_c(cord_num,cord_num + 1,nucl_num,elec_num,walk_num)
      real    (c_double ) , intent(in)          :: dtmp_c(cord_num,cord_num + 1,nucl_num,4,elec_num,walk_num)
      real    (c_double ) , intent(in)          :: een_rescaled_n(0:cord_num,nucl_num,elec_num,walk_num)
      real    (c_double ) , intent(in)          :: een_rescaled_n_deriv_e(0:cord_num,nucl_num,4,elec_num,walk_num)
      real    (c_double ) , intent(out)         :: factor_een_deriv_e(elec_num,4,walk_num)
