...
<<pre2>>
  ...
  if (ctx->jastrow.aord_vector != NULL) {
    ...
    if (rc != QMCKL_SUCCESS) {
      return qmckl_failwith( context, rc,
                             "qmckl_set_jastrow_aord_vector",
                             "Unable to free ctx->jastrow.aord_vector");
    }
  }
...
<<pre2>>
  ...
  if (ctx->jastrow.bord_vector != NULL) {
    ...
    if (rc != QMCKL_SUCCESS) {
      return qmckl_failwith( context, rc,
                             "qmckl_set_jastrow_bord_vector",
                             "Unable to free ctx->jastrow.bord_vector");
    }
  }
...
<<pre2>>
  ...
  if (ctx->jastrow.cord_vector != NULL) {
    ...
    if (rc != QMCKL_SUCCESS) {
      return qmckl_failwith( context, rc,
                             "qmckl_set_jastrow_cord_vector",
                             "Unable to free ctx->jastrow.cord_vector");
    }
  }
...
** Asymptotic component for \(J_{ee}\)
  ...
  via the ~bord_vector~ and the electron-electron rescale factor ~rescale_factor_ee~.
...
integer function qmckl_compute_asymp_jasb_f(context, bord_num, bord_vector, rescale_factor_ee, asymp_jasb) &
     result(info)
  use qmckl
  implicit none
