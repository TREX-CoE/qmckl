...
The Point data structure is a flexible container for storing sets of 3D
Cartesian coordinates at which various quantum chemical quantities will be
evaluated. This abstraction allows the same computational kernels to be used
in different contexts without modification.

In Density Functional Theory (DFT) codes, points typically represent the
integration grid points used for numerical integration of exchange-correlation
functionals and electron density. In Quantum Monte Carlo (QMC) codes, these
points represent the electron coordinates of all the Monte Carlo walkers
being sampled simultaneously.

By providing a unified representation for evaluation points, the library
enables code reuse and simplifies the implementation of quantum chemistry
methods that need to evaluate wavefunctions, orbitals, or other quantities
