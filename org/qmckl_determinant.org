...
The Slater determinant is a fundamental component of the quantum mechanical
wavefunction in electronic structure calculations. It represents the antisymmetric
nature of the fermionic wavefunction and is essential for satisfying the Pauli
exclusion principle. The Slater determinant and its derivatives are required for 
calculating the wavefunction value, its gradient, and its Laplacian, which in turn
are needed to compute the local energy ($E_L$) in quantum Monte Carlo simulations.

The Slater determinant is defined as:

ψ(x) = det|ϕ₁(x₁)...ϕᵢ(xᵢ)...ϕₙ(xₙ)|

This represents the determinant of a matrix where each column corresponds to a
molecular orbital evaluated at the position of a specific electron.

The Slater matrix (denoted by $D_{ij}(x)$) is the matrix whose determinant
gives the wavefunction:

ψ(x) = det|Dᵢⱼ(x)|

A key computational quantity in QMC is the inverse of the Slater matrix,
denoted by $D^{-1}_{ij}(x)$. The inverse is crucial for efficiently computing
acceptance probabilities in Metropolis Monte Carlo sampling.

Using this inverse matrix, the ratio of wavefunctions at two different
electronic configurations (which determines the acceptance probability) can be
computed efficiently without calculating determinants directly:

ψ(yᵢ)/ψ(xᵢ) = ∑ⱼDᵢⱼ(y)D⁻¹ⱼᵢ(x)

This formula enables the efficient evaluation of single-electron moves in the
Monte Carlo algorithm.

Concerning the gradient and Laplacian needed for computing the local energy,
the quantities actually computed are the logarithmic derivatives: the ratio of 
the gradient (or Laplacian) to the determinant of the Slater matrix:
...
Computing these ratios directly is both more numerically stable and more
efficient than computing the gradient and Laplacian separately, as it avoids
the explicit calculation and subsequent division by the potentially very large
or very small determinant value ψ(x). These logarithmic derivatives are the
natural quantities appearing in the local energy expression.
...
integer function qmckl_compute_det_vgl_alpha_f(context, &
  ...
  do iwalk = 1, walk_num
    ...
    #+CALL: generate_private_private_c_header(table=qmckl_compute_det_vgl_alpha_args,rettyp=get_value("CRetType"),fname="qmckl_compute_det_vgl_alpha"))
   ...
   #+CALL: generate_private_private_c_interface(table=qmckl_compute_det_vgl_alpha_args,rettyp=get_value("CRetType"),fname="qmckl_compute_det_vgl_alpha"))
...
integer function qmckl_compute_det_vgl_beta_f(context, &
  ...
  do iwalk = 1, walk_num
    ...
    #+CALL: generate_private_private_c_header(table=qmckl_compute_det_vgl_beta_args,rettyp=get_value("CRetType"),fname="qmckl_compute_det_vgl_beta"))
   ...
   #+CALL: generate_private_private_c_interface(table=qmckl_compute_det_vgl_beta_args,rettyp=get_value("CRetType"),fname="qmckl_compute_det_vgl_beta"))
...
integer function qmckl_compute_det_inv_matrix_alpha_f(context, &
  ...
  deallocate(matA)
   ...
   #+end_src
    ...
    #+CALL: generate_private_c_interface(table=qmckl_det_inv_matrix_alpha_args,rettyp=get_value("CRetType"),fname="qmckl_compute_det_inv_matrix_alpha"))
...
integer function qmckl_compute_det_inv_matrix_beta_f(context, &
  ...
  deallocate(matA)
   ...
   #+end_src
    ...
    #+CALL: generate_private_c_interface(table=qmckl_det_inv_matrix_beta_args,rettyp=get_value("CRetType"),fname="qmckl_compute_det_inv_matrix_beta"))
