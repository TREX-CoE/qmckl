...
** OpenMP thread count
   ...
   # Header
   #+begin_src c :comments org :tangle (eval h_func)
int32_t qmckl_context_get_num_threads(const qmckl_context context);
   #+end_src
...
int32_t qmckl_context_get_num_threads(const qmckl_context context) {
  if (qmckl_context_check(context) == QMCKL_NULL_CONTEXT) {
    return (int32_t) qmckl_failwith(context,
                                    QMCKL_INVALID_CONTEXT,
  ...
  return ctx->num_threads;
   ...
   #+begin_src f90 :tangle (eval fh_func) :exports none
   interface
      integer (c_int32_t) function qmckl_context_get_num_threads(context) bind(C)
        use, intrinsic :: iso_c_binding
        import
        integer (qmckl_context), intent(in), value :: context
...
{
  int32_t initial_num_threads = qmckl_context_get_num_threads(context);
  assert(initial_num_threads > 0);

  assert(qmckl_context_set_num_threads(context, 1) == QMCKL_SUCCESS);
  assert(qmckl_context_get_num_threads(context) == 1);

  assert(qmckl_context_set_num_threads(context, 2) == QMCKL_SUCCESS);
  assert(qmckl_context_get_num_threads(context) == 2);

  assert(qmckl_context_unset_num_threads(context) == QMCKL_SUCCESS);
  assert(qmckl_context_get_num_threads(context) == initial_num_threads);
  ...
  assert(qmckl_context_get_num_threads(QMCKL_NULL_CONTEXT) < 0);
