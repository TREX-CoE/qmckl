...
qmckl_context qmckl_context_copy(const qmckl_context context) {
  ...
  qmckl_lock(context);
  {
    ...
    /* Deep copy memory allocations */
    for (size_t pos = (size_t) 0 ; pos < old_ctx->memory.array_size ; ++pos) {
      if (old_ctx->memory.element[pos].pointer != NULL) {
        size_t size = old_ctx->memory.element[pos].size;
        qmckl_memory_info_struct mem_info = qmckl_memory_info_struct_zero;
        mem_info.size = size;
        void* new_ptr = qmckl_malloc(new_context, mem_info);
        if (new_ptr == NULL) {
          qmckl_context_destroy(new_context);
          qmckl_unlock(context);
          return QMCKL_NULL_CONTEXT;
        }
        memcpy(new_ptr, old_ctx->memory.element[pos].pointer, size);
      }
    }
